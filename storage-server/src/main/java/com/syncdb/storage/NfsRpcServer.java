...
public class NfsRpcServer implements Closeable {
  private final OncRpcSvc nfsSvc;
  @Getter private final Path root;
  @Getter private final int port;
  private final String name;

  public NfsRpcServer(Path root) {
    this(0, 2049, root, null, null);
  }

  public NfsRpcServer(int nfsVers, int port, Path root, ExportFile exportFile, String name) {
    try {
            ...
            new ExportFile(
                new InputStreamReader(
                    Objects.requireNonNull(
                        NfsRpcServer.class.getClassLoader().getResourceAsStream("exports"))));
      }
      ...
      VirtualFileSystem vfs =
          new LocalFileSystem(this.root, exportFile.exports().collect(Collectors.toList()));
